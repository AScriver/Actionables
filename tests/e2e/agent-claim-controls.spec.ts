  ...
  const cases = [
    {
      ...
      prompt: `Use Actionables work item #${original.id}. Claim task #${original.id} and continue from Ready. Use the task detail returned by the Actionables MCP as the authoritative source for the recorded finding, existing research, sources, file references, relationships, and planned validation. ${TRUNCATION_INSTRUCTIONS} ${COMPOSED_TOOL_INSTRUCTIONS} ${READINESS_INSTRUCTIONS} Confirm this task remains the coordination record for its subtree; do not implement or duplicate any child task's scope. Use the child inventory to confirm every required descendant is terminal, and hand off with the coordination blocker if any remain nonterminal. Otherwise move this task to In progress before finalizing it, preserve existing user modifications, run the planned validation, populate Resolution with the completed changes and important implementation decisions, record qualifying validation evidence, and only then move #${original.id} to Done; otherwise hand off with the blocker. If implementation uncovers a need for more investigation, return In progress directly to Researching with a meaningful reason.`,
    },
    {
      status: "In progress" as const,
      prompt: `Use Actionables work item #${original.id}. Claim task #${original.id} and resume implementation from In progress. Use the task detail returned by the Actionables MCP as the authoritative source for the recorded finding, existing research, sources, file references, relationships, and planned validation. ${TRUNCATION_INSTRUCTIONS} ${COMPOSED_TOOL_INSTRUCTIONS} Confirm this task remains the coordination record for its subtree; do not implement or duplicate any child task's scope. Use the child inventory to confirm every required descendant is terminal, and hand off with the coordination blocker if any remain nonterminal. Otherwise finalize this task, preserve existing user modifications, run the planned validation, populate Resolution with the completed changes and important implementation decisions, record qualifying validation evidence, and only then move #${original.id} to Done; otherwise hand off with the blocker. If implementation uncovers a need for more investigation, return In progress directly to Researching with a meaningful reason.`,
    },
  ];
