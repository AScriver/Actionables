...
test("invalid capture preserves entered values and exposes accessible field errors", async ({ page }) => {
  await page.goto("/");
  ...
  await expect(page.getByLabel("Description")).toHaveValue("Keep this unsaved description.");
  ...
  await expect(page.getByRole("dialog", { name: "New actionable" })).toBeVisible();
  await expect(page.getByLabel("Description")).toHaveValue("Keep this unsaved description.");
});

test("a stale save keeps the second draft recoverable across two browser contexts", async ({ browser }) => {
  ...
  await expect(firstPage.getByRole("heading", { name: "Two-context conflict test" })).toBeVisible();

  await secondPage.goto(deepLink);
  await expect(secondPage.getByRole("heading", { name: "Two-context conflict test" })).toBeVisible();
  ...
  await secondPage.getByLabel("Title").fill("Recoverable draft from the second browser");

  await firstPage.getByRole("button", { name: "Save changes" }).click();
  await expect(firstPage.getByRole("heading", { name: "Saved from the first browser" })).toBeVisible();
  ...
  await expect(conflict).toContainText("Your unsaved draft has not been changed or discarded");
  await expect(secondPage.getByLabel("Title")).toHaveValue(
    "Recoverable draft from the second browser",
  );

  await secondPage.getByRole("button", { name: "Review current saved version" }).click();
  await expect(conflict).toContainText("Saved from the first browser");
  await secondPage.getByRole("button", { name: "Reload version and reapply draft" }).click();
  await expect(secondPage.getByLabel("Title")).toHaveValue(
    "Recoverable draft from the second browser",
  );
  await secondPage.getByRole("button", { name: "Save changes" }).click();
  await expect(
    secondPage.getByRole("heading", { name: "Recoverable draft from the second browser" }),
  ).toBeVisible();
...
test("a mobile deep link opens detail and returns to the usable list", async ({ page }) => {
  ...
  await expect(page.getByRole("table", { name: "Actionable findings" })).toBeVisible();
  await expect(page.getByRole("button", { name: "New actionable" })).toBeVisible();
});

test("triage to Ready persists the required fields and server-approved status", async ({ page }) => {
  await page.goto("/");
  await page.getByRole("button", { name: "New actionable" }).click();
  ...
  await expect(page.getByText("The browser flow has a concrete finding.")).toBeVisible();
  await expect(inspector.getByLabel(/^Ready\./)).toBeVisible();
});
