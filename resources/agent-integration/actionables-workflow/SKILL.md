...
- Read research, Resolution and stored references together with `actionables.get_task_history`, using the result's `id`, top-level `workItemId` and `version`. Known Done or Dismissed tasks can use the version from `get_task`. Start at offset 0; preserve that version, the first `contentHash` and the same archive option with each returned `nextOffset` until null. Each page contains at most 40 items and 8,000 serialized characters of items, plus bounded metadata.
