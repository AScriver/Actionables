...
function createActionablesMcpServer(prisma: AppPrismaClient) {
  ...
  server.registerTool(
    "actionables.create_task",
    {
      ...
      description:
        "Create one task and return its detail. For a top-level task, either provide projectId, repositoryId, and worktreeId or provide repositoryPath with ensureScope true to resolve and provision the local Git scope. For one direct subtask, provide the authorized top-level Actionable as both workItemId and parentId, and omit placement fields. Reuse the idempotency UUID only for an exact retry.",
